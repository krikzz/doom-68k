...
void WI_slamBackground(void) {
    std_memcpy(screens[0], screens[1], SCREENWIDTH * SCREENHEIGHT);
...
doomboolean WI_Responder(event_t* ev) {
    return false;
}
...
(int n,
    ...
    if (fits && i < 2) {
        ...
        // DEBUG
        std_printf("Could not place patch on level %d", n + 1);
    }
}
...
static doomboolean snl_pointeron = false;
...
void WI_drawStats(void) {
    ...
    V_DrawPatch(SP_TIMEX, SP_TIMEY, FB, dtime);
...
void WI_loadData(void) {
    ...
    if (gamemode == commercial)
        std_strcpy(name, "INTERPIC");
    else
        std_sprintf(name, "WIMAP%d", wbs->epsd);
    ...
    if (gamemode == commercial) {
        ...
        lnames = (patch_t **) Z_Malloc(sizeof (patch_t*) * NUMMAPS,
                PU_STATIC, 0);
        for (i = 0; i < NUMMAPS; i++) {
            std_sprintf(name, "WILV%d%d", wbs->epsd, i);
            lnames[i] = W_CacheLumpName(name, PU_STATIC);
        }
        ...
        if (wbs->epsd < 3) {
            for (j = 0; j < NUMANIMS[wbs->epsd]; j++) {
                ...
                for (i = 0; i < a->nanims; i++) {
                    ...
                    if (wbs->epsd != 1 || j != 8) {
                        // animations
                        std_sprintf(name, "WIA%d%.2d%.2d", wbs->epsd, j, i);
    ...
    // "time"
    dtime = W_CacheLumpName("WITIME", PU_STATIC);
    ...
    for (i = 0; i < MAXPLAYERS; i++) {
        // "1,2,3,4"
        std_sprintf(name, "STPB%d", i);
        p[i] = W_CacheLumpName(name, PU_STATIC);

        // "1,2,3,4"
        std_sprintf(name, "WIBP%d", i + 1);
