...
u8 bi_init() {
    ...
    halt_app_ram = (u8 *) 0xff8000; //must be located in megadrive ram
    std_memcpy(halt_app_ram, bi_halt_app, app_len);
...
void bi_cmd_file_read_nb(u32 addr, u32 len) {
...
u8 bi_cmd_file_write(void *src, u32 len) {
    ...
    while (len) {
        ...
        block = std_min(ACK_BLOCK_SIZE, len);
...
void bi_cmd_mem_wr(u32 addr, void *src, u32 len) {
    ...
    while (len) {
        ...
        block = std_min(len, ACK_BLOCK_SIZE);
