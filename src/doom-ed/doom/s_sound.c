...
(int sfxVolume,
    ...
    std_fprintf(stderr, "S_Init: default sfx volume %d\n", sfxVolume);
        ...
        if (sfx_id == sfx_sawidl
            ...
            for (i = first_saw; i != next_saw; i = (i + 1) % 10)
                if (last_saw_origins[i] != origin)
                    std_fprintf(stderr, "old origin 0x%lx != "
...
(int musicnum,
    ...
    if (!music->lumpnum) {
        std_sprintf(namebuf, "d_%s", music->name);
...
(mobj_t* listener,
    ...
    adx = std_abs(listener->x - source->x);
    ady = std_abs(listener->y - source->y);
